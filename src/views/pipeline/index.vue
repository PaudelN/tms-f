          ...
          <template #filter-extra="{ draft, on }">
            <div class="space-y-2.5">
              ...
              <div class="flex flex-wrap gap-2">
                ...
                >
                  <span class="h-2 w-2 rounded-full shrink-0" :class="status.dot" />
                  {{ status.label }}
                  <span
      ...
      >
        ...
        >
          ...
          <template #cell-stages_count="{ row }">
            <span class="text-sm tabular-nums">{{ row.stages_count ?? 0 }}</span>
          </template>
          ...
          <template #item-expanded="{ item }">
            <div class="space-y-3">
              <div class="grid grid-cols-3 gap-3">
                ...
                <div>
                  <p class="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-0.5">Stages</p>
        ...
        <template v-else-if="currentView === 'kanban'">
          ...
          <div class="flex-1 min-h-0 relative">
            ...
            >
              <div class="h-12 w-12 rounded-2xl bg-muted/50 flex items-center justify-center">
              ...
              <button
                ...
                @click="kanbanActivePipelineId && handleView(kanbanActivePipelineId)"
              >
            ...
            >
              <!-- Fake task card — swap this slot for a real TaskCard component later -->
              <template #card="{ item }">
                <div class="flex flex-col gap-2">
                  <div class="flex items-start gap-2">
                    <p class="flex-1 text-sm font-semibold text-foreground leading-snug">
                      {{ item.title }}
                    </p>
                    <span
                      class="shrink-0 text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-md"
                      :class="item._priorityClass"
                    >{{ item._priority }}</span>
                  </div>
                  ...
                  <div class="flex items-center gap-2 pt-0.5">
                    <span
                      class="h-5 w-5 rounded-full flex items-center justify-center text-[9px] font-bold text-white shrink-0"
                      :style="{ background: item._avatarColor }"
                    >{{ item._avatarInitial }}</span>
                    <span class="text-[10px] text-muted-foreground truncate flex-1">{{ item._assignee }}</span>
                    <span class="text-[10px] text-muted-foreground shrink-0">{{ item._due }}</span>
                  </div>
                </div>
              </template>
...
function onViewChange(view: ViewMode) {
  ...
  if (view === "kanban" && kanbanStages.value.length === 0 && !kanbanStagesLoading.value) {
    initKanban()
  }
}
...
function tableFetchFn(params: UniversalFetchParams) {
  return pipelineStore.fetchPipelines({ ...params, projectId: projectId.value })
}
...
const PIPELINE_COLORS = [
  "#7c6ff7", "#f6a623", "#34c789", "#4a9eff",
  "#ff6b9d", "#a855f7", "#00c4cc", "#8bc34a",
]
...
async function loadKanbanPipelineList(): Promise<void> {
  try {
    const res = await pipelineStore.fetchPipelines({
      ...
      projectId: projectId.value, filters: null,
    })
    ...
    if (kanbanPipelineList.value.length > 0 && !kanbanActivePipelineId.value) {
      kanbanActivePipelineId.value = kanbanPipelineList.value[0].id
    }
  } catch {
    notify.error("Failed to load pipelines", "", { position: "bottom-right" })
  }
}
...
function onKanbanPipelineSwitch(id: number): void {
  ...
  // Clear stages + fake task cache so fresh data loads for the new pipeline
  rawStages.value = []
  fakeTasks.value = {}
  reloadKanbanStages()
}
...
async function reloadKanbanStages(): Promise<void> {
  ...
  try {
    ...
    // Pre-populate fake tasks whenever stages refresh
    seedFakeTasks()
  } finally {
...
const FAKE_TITLES = [
  "Update onboarding flow",   "Fix login redirect bug",  "Write API docs",
  "Design new dashboard",     "Code review PR #42",      "Add unit tests",
  "Migrate to v3 schema",     "Audit permissions",       "Deploy to staging",
  "Sync with design team",    "Refactor auth module",    "QA regression run",
]
const FAKE_DESCS = [
  "Needs UX sign-off before dev.", "Reported by 3 customers.", "",
  "High priority — due end of sprint.", "", "Cover edge cases for null inputs.",
]
...
function kanbanFetchFn(params: UniversalFetchParams) {
  const stageId = params.kanbanStage ?? ""
  const bucket  = fakeTasks.value[stageId] ?? []

  const q        = (params.search ?? "").trim().toLowerCase()
  const filtered = q
    ? bucket.filter((t) => t.title.toLowerCase().includes(q))
    : bucket

  return Promise.resolve({
    data: filtered,
    meta: { current_page: 1, last_page: 1, total: filtered.length, per_page: 200 },
  })
}
...
// ── @move — optimistic local update ──────────────────────────────────────────
// Moves a fake task between stage buckets. No API call yet.
// ↳ When real tasks exist: call your task kanban move endpoint here.
function onKanbanMove(event: KanbanMoveEvent<FakeTask>): void {
  const from = [...(fakeTasks.value[event.fromStage] ?? [])]
  const to   = [...(fakeTasks.value[event.toStage]   ?? [])]

  const idx = from.findIndex((t) => t.id === event.item.id)
  if (idx === -1) return

  const [task] = from.splice(idx, 1)
  to.splice(event.newIndex, 0, task)
...
// ── @reorder — persists column order via the existing stages/reorder endpoint ─
// UiKanban emits orderedIds = stage IDs in the new order (within one column).
// But here, the "stage" value IS the column — so orderedIds is the full new
// column order when the user drags a column strip.
//
// The PipelineStageController.reorder() expects:
//   { stages: [{ id: number, display_order: number }] }
//
// ↳ When real tasks exist, intra-stage card reorder fires here too — distinguish
//   by checking whether event.orderedIds contains stage IDs or task IDs.
async function onKanbanReorder(event: KanbanReorderEvent): Promise<void> {
  const pid = kanbanActivePipelineId.value
  if (!pid) return

  // Map the new ordered list of stage IDs → { id, display_order } pairs
  const payload = {
    stages: event.orderedIds.map((id, index) => ({
      id:            Number(id),
      display_order: index,
    })),
  }
  ...
  try {
    await pipelineStageStore.reorderStages(pid, payload)
    // Sync local rawStages order to match what we just persisted
    const idOrder = event.orderedIds.map(String)
    rawStages.value = [...rawStages.value].sort(
      (a, b) => idOrder.indexOf(String(a.id)) - idOrder.indexOf(String(b.id))
    )
  } catch {
    notify.error("Reorder failed", "Could not save stage order.", { position: "bottom-right" })
    // Rollback: re-fetch authoritative order from server
    reloadKanbanStages()
  }
}
...
function extractStatusLabel(status: Pipeline["status"]): string {
  ...
  if (typeof status === "object" && "label" in status!) return (status as any).label
  const val = extractStatusValue(status)
  return pipelineStore.statuses.find((s) => s.value === val)?.label ?? String(val)
}
function extractStatusDot(status: Pipeline["status"]): string {
  if (typeof status === "object" && status !== null && "dot" in status) return (status as any).dot
  const val = extractStatusValue(status)
  return pipelineStore.statuses.find((s) => s.value === val)?.dot ?? "bg-muted-foreground"
}
function extractStatusBadge(status: Pipeline["status"]): string {
  if (typeof status === "object" && status !== null && "badge" in status) return (status as any).badge
  const val = extractStatusValue(status)
...
const tableConfig: TableConfig = {
  defaultPerPage: 10, defaultSortBy: "created_at",
  defaultSortOrder: "desc", debounceMs: 400, persistState: true,
}
...
async function confirmDelete() {
  ...
  try {
    ...
    notify.success("Pipeline deleted", "The pipeline was removed successfully.", { position: "bottom-right" })
    onRefresh()
  } catch {
    notify.error("Delete failed", "We couldn't delete the pipeline.", { position: "bottom-right" })
  } finally {
