          ...
          >
              ...
              <div class="flex items-center gap-3">
                <div class="h-10 w-10 rounded-xl bg-primary/20 flex items-center justify-center relative">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="icon"
                    class="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-card border border-border hover:bg-primary hover:border-primary"
            ...
            <template #cell-actions="{ row }">
              <div class="flex items-center justify-center gap-1">
                <Button variant="ghost" size="icon" title="View" @click="handleView(row.id)">
                  <Eye class="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="icon" title="Edit" @click="handleEdit(row.id)">
              ...
              <div class="flex items-center justify-between p-4 hover:bg-accent transition-colors">
                ...
                <div class="flex items-center gap-3 flex-shrink-0 ml-4">
                  ...
                  <div class="flex gap-1">
                    <Button variant="ghost" size="icon" @click.stop="togglePin(item.id)">
                      <Star class="h-4 w-4" :class="isPinned(item.id) ? 'text-primary' : 'text-muted-foreground'" />
                    </Button>
                    <Button variant="ghost" size="icon" class="text-primary" @click="handleView(item.id)">
            ...
            <template #item="{ items }">
              ...
              >
                <div class="flex items-start justify-between mb-3">
                  ...
                  >
                    <Star class="h-3.5 w-3.5" :class="isPinned(item.id) ? 'text-primary' : 'text-muted-foreground'" />
                  </Button>
                </div>
                ...
                <div class="flex items-center justify-between pt-3 border-t border-border">
                  <span class="text-xs text-muted-foreground">{{ formatDate(item.created_at) }}</span>
                  <div class="flex gap-1">
                    <Button variant="ghost" size="icon" @click.stop="handleEdit(item.id)">
                      <Pencil class="h-3.5 w-3.5 text-foreground" />
                    </Button>
                    <Button variant="ghost" size="icon" @click.stop="toggleArchive(item.id)">
...
function exportData(format: "csv" | "json") {
  ...
  toastSuccess("Export complete", `Saved ${format.toUpperCase()} file`);
}
...
function copyShareLink() {
  ...
  toastSuccess("Link copied", "Invite link copied to clipboard");
}
