            ...
            <div class="flex flex-wrap gap-2">
              ...
              >
                <span
                  ...
                  :class="STAGE_STYLE[status.value]?.dot ?? 'bg-muted-foreground'"
                />
                {{ status.label }}
                <span
      ...
      <div class="flex-1 min-h-0 flex flex-col">
        ...
        >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center justify-center gap-1" @click.stop>
              <button
                type="button"
                class="h-7 w-7 rounded-lg flex items-center justify-center text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors"
        ...
        >
          ...
          <template #item-actions="{ item }">
            <button
              type="button"
              class="h-7 w-7 rounded-lg flex items-center justify-center text-primary opacity-0 group-hover/row:opacity-100 transition-opacity"
            ...
            <div class="space-y-3">
              ...
              <div class="flex gap-2 pt-1">
                ...
                <button
                  ...
                  @click="confirmDeletePrompt(item.id, item.name)"
                >
...
<script setup lang="ts">
  ...
  // ── Kanban HTTP API (move + reorder only) ─────────────────────────────────────
  const kanbanApi = useKanbanApi<Workspace>("/api/workspaces");
  ...
  const filterFields: FilterField[] = [
    {
      ...
      label: "Date Order",
      type: "select",
      description: "Sort workspaces by their creation date.",
      options: [
    ...
    },
    {
      ...
      label: "Created From",
      type: "date",
      description: "Show workspaces created on or after this date.",
    },
    {
      key: "created_to",
      label: "Created To",
      type: "date",
      description: "Show workspaces created on or before this date.",
    },
    {
      key: "creator",
      label: "Owner",
  ...
  // ── Universal interactions — search + filter shared across all three views ─────
  const {
    searchQuery,
    activeFilterCount,
    handleSearch,
