      ...
      <Card v-if="workspace" class="border-border">
        ...
        <CardContent class="px-5 pb-5 pt-2 space-y-3">
          ...
          <div>
            ...
            <dd>
              <!-- Fallback: show raw status value when statuses haven't loaded yet -->
              <span
...
// Safe: returns undefined when statuses not yet loaded — no crash
const selectedStatus = computed(() =>
  workspaceStore.statuses.find((s) => s.value === values.status),
)
const currentStatusObj = computed(() =>
  workspaceStore.statuses.find((s) => s.value === (workspace.value as any)?.status),
)
