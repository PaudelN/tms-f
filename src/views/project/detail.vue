<template>
  ...
  >
    <!-- ────────────────────────────────────────────────────────────────────
         Detail tab — Cover + Description (unchanged)
    ──────────────────────────────────────────────────────────────────────── -->
    <template #content>
    ...
    <template #tab-pipelines>
      ...
      >
        ...
        <template #item-summary="{ item: pl }">
          <div class="flex items-center gap-3 min-w-0">
            <div class="h-8 w-8 rounded-lg bg-primary/10 flex items-center justify-center shrink-0 group-hover/row:bg-primary/15 transition-colors">
            ...
            <div v-if="pl.stages_count != null" class="hidden sm:flex items-center gap-1 shrink-0">
            ...
            <ChevronRight class="h-3.5 w-3.5 text-muted-foreground/30 shrink-0 group-hover/row:text-primary/50 transition-colors" />
          </div>
        </template>
        ...
        <template #item-expanded="{ item: pl }">
          <div class="grid grid-cols-2 gap-x-6 gap-y-3 py-1">
            <div v-if="pl.description" class="col-span-2">
              <p class="text-[10px] font-semibold tracking-[0.07em] uppercase text-muted-foreground/50 mb-1">
                Description
              </p>
              <p class="text-[12px] text-foreground/70 leading-relaxed">{{ pl.description }}</p>
            </div>

            <div v-if="pl.stages_count != null">
              <p class="text-[10px] font-semibold tracking-[0.07em] uppercase text-muted-foreground/50 mb-1">Stages</p>
              <div class="flex items-center gap-1.5">
                <Layers class="h-3 w-3 text-muted-foreground/40" />
                <span class="text-[12px] font-medium text-foreground/80 tabular-nums">{{ pl.stages_count }}</span>
              </div>
            </div>

            <div>
              <p class="text-[10px] font-semibold tracking-[0.07em] uppercase text-muted-foreground/50 mb-1">Status</p>
              <span
      ...
      <div v-else class="flex flex-col h-full">
        <div class="h-[46px] border-b border-border bg-card/50 animate-pulse shrink-0" />
        <div
          v-for="i in 5"
          :key="i"
...
const detailTabs = computed<TabItem[]>(() => [
  {
    ...
    badge: (project.value as any)?.active_pipelines_count
