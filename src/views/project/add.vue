  ...
  >
    ...
    <template #fields>
      <form @submit.prevent>
          ...
          <div class="flex items-start gap-4">
            ...
            <div>
              ...
              <p class="text-[13px] text-muted-foreground mt-0.5">
                Create a new project inside this workspace to organise your team's work.
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="px-8 py-8">
          <div class="grid grid-cols-1 sm:grid-cols-[1fr_200px] gap-3 items-start">
            <FormField v-slot="{ componentField, meta: fieldMeta }" name="name">
              ...
              >
                <Input
                  placeholder="e.g. Platform Redesign, Q4 Marketing"
        ...
        <!-- Visibility + Dates row -->
        <div class="px-8 pb-6">
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-start">
            <FormField v-slot="{ componentField }" name="visibility">
              <UiFormField label="Visibility" required :icon="Eye">
            ...
            <FormField v-slot="{ componentField }" name="start_date">
              <UiFormField label="Start Date" badge="Optional" :icon="CalendarDays">
                <Input
                  type="date"
                  class="h-10 bg-muted border-border focus-visible:bg-card transition-colors text-[13px]"
                  v-bind="componentField"
            ...
            <FormField v-slot="{ componentField }" name="end_date">
              <UiFormField label="End Date" badge="Optional" :icon="CalendarDays">
                <Input
                  type="date"
                  class="h-10 bg-muted border-border focus-visible:bg-card transition-colors text-[13px]"
                  v-bind="componentField"
        ...
        <div class="px-8 py-1">
          <FormField v-slot="{ componentField }" name="description">
            <UiFormField
              label="Description"
              badge="Optional"
              hint="Describe the project goals, scope, and key deliverables."
              :icon="FileText"
            >
              <Textarea
...
// workspaceId comes from the nested route: /workspace/:workspaceId/projects/add
...
const tips = [
  "Pick a name that clearly communicates the project's goal.",
  'Set status to "Draft" while planning, switch to "In Progress" when work begins.',
  "Visibility controls who in the workspace can see this project.",
  "Start and end dates help with scheduling across the workspace.",
];
...
const onSubmit = handleSubmit(async (formValues) => {
  ...
  try {
    const project = await projectStore.createProject(workspaceId.value, {
    ...
    notify.success("Project created", `"${project.name}" is now ready to use.`);
    // Navigate to shallow project detail
    router.push({ name: "project-detail", params: { id: project.id } });
  } catch (e) {
    notify.error("Failed to create project", "Please check the form and try again.");
    console.error(e);
  }
});
...
const breadcrumbs: FormBreadcrumb[] = [
  { label: "Workspaces", onClick: () => router.push({ name: "workspace" }) },
  {
    label: "Projects",
    onClick: () =>
      router.push({ name: "project-index", params: { workspaceId: workspaceId.value } }),
  },
  { label: "New Project" },
];
</script>