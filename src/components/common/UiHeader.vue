  ...
  <div class="mb-8">
    <div class="flex items-center justify-between gap-4">
      <div class="space-y-1.5">
        <h1 class="text-2xl font-bold text-primary tracking-wide">
          {{ title }}
        </h1>
        ...
        <Tooltip v-if="showRefresh">
          <TooltipTrigger as-child>
            <button
              type="button"
              :disabled="loading"
              class="relative cursor-pointer flex items-center justify-center w-10 h-10 mr-2 rounded-[4px] transition-all duration-200 bg-primary-20 text-primary"
              @click="emit('refresh')"
            >
              <RefreshCcw
